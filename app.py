...
def uprava_pojisteni(id):
    ...
    if request.method == 'GET':
        ...
        result_pojistenec = pojistenec_cur.fetchone()

        pojisteni_cur = db.execute('SELECT predmet_pojisteni FROM pojisteni')
        result_pojisteni = pojisteni_cur.fetchall()
        ...
        db.execute('UPDATE pojistky SET predmet_pojisteni = ?, typ_pojisteni = ?, castka = ?, platnost_od = ?, platnost_do = ?  WHERE id = ?',[typ_pojisteni, predmet_pojisteni, hodnota_pojisteni, platnost_od, platnost_do, id_pojistence])
        db.commit()
...
def pojistenci_detail(id):
    ...
    return render_template('pojistenec_detail.html', page_title='Pojištěnec {}'.format(result['prijmeni']), data = result, pojisteni = pojisteni_result)
